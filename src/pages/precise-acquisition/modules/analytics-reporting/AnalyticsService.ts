...
export class AnalyticsService {
  ...
  async getTrendData(
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      data.push({
        ...
        percentage: previousValue ? ((value - previousValue) / previousValue) * 100 : 0
      });
    }
   ...
   * 获取平台指标
   */
  async getPlatformMetrics(platform: 'xiaohongshu' | 'douyin'): Promise<PlatformMetrics> {
    const platformData: PlatformMetrics = {
      ...
      stats: {
        ...
        audienceInsights: {
          ...
          interests: [
            { category: '美妆护肤', count: this.generateRandomValue(100, 300), percentage: 0 },
            { category: '时尚穿搭', count: this.generateRandomValue(80, 250), percentage: 0 },
            { category: '生活方式', count: this.generateRandomValue(60, 200), percentage: 0 }
          ]
        }
      },
  ...
  private generateRecommendations(
    ...
    // 效率建议
    if (current.effectiveness.engagementRate < 20) {
      recommendations.push({
        type: 'efficiency',
        priority: 'medium',
        title: '提升互动效果',
  ...
  private generateQueryResult(query: AnalyticsQuery): Array<Record<string, any>> {
  ...
  private calculateAggregations(
    ...
    metrics.forEach(metric => {
      ...
      aggregations[`${metric}_avg`] = values.length > 0 ? aggregations[`${metric}_sum`] / values.length : 0;
  ...
  private metricsToCSV(metrics: ReportMetrics): string {
    ...
    const rows = [
      ...
      ['总目标数', metrics.effectiveness.targets.totalTargets.toString(), '效果分析'],
      ['互动率', `${metrics.effectiveness.targets.engagementRate}%`, '效果分析'],
