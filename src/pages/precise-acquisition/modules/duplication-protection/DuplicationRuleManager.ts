...
export class DuplicationRuleManager {
   ...
   * 更新规则
   */
  updateRule(ruleId: string, updates: Partial<DuplicationRule>): DuplicationRule | null {
   ...
   * 验证规则配置
   */
  validateRule(rule: Partial<DuplicationRule>): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    if (rule.timeWindow) {
      ...
      if (!['minutes', 'hours', 'days', 'weeks'].includes(rule.timeWindow.unit)) {
        errors.push('时间单位无效');
      }
    }

    if (rule.conditions) {
      if (rule.conditions.maxActionsPerTarget !== undefined && rule.conditions.maxActionsPerTarget < 1) {
        errors.push('每目标最大操作次数必须≥1');
      }
      if (rule.conditions.maxActionsPerTimeWindow !== undefined && rule.conditions.maxActionsPerTimeWindow < 1) {
        errors.push('时间窗口内最大操作次数必须≥1');
      }
      if (rule.conditions.cooldownPeriod !== undefined && rule.conditions.cooldownPeriod < 0) {
   ...
   * 导入规则
   */
  importRules(rules: DuplicationRule[]): { imported: number; errors: string[] } {
    let imported = 0;
    const errors: string[] = [];
   ...
   * 工具方法：分组统计
   */
  private groupBy<T>(array: T[], keySelector: string | ((item: T) => string)): Record<string, number> {
