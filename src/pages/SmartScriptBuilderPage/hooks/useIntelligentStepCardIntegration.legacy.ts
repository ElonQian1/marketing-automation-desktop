// src/pages/SmartScriptBuilderPage/hooks/useIntelligentStepCardIntegration.legacy.ts
// module: pages | layer: hooks | role: legacy-backup
// summary: 智能步骤卡集成Hook旧版备份，供回滚参考
  ...
  const handleQuickCreateStep = useCallback(
    async (element: UIElement | VisualUIElement) => {
      try {
        ...
        // 🆕 智能生成步骤标题：优先使用有意义的文本信息
        // 对于透明交互层，优先使用父元素的完整描述，次要使用子元素文本
        const stepTitle = (() => {
          // 1. 优先使用元素自身的文本
          if (uiElement.text) return uiElement.text;
          // 2. 优先使用元素自身的 content_desc
          if (uiElement.content_desc) return uiElement.content_desc;
          // 3. 🆕 使用父元素的 content_desc（瀑布流卡片通常在父元素有完整描述）
          if (context.parentElement?.content_desc) {
            // 截取前20个字符作为标题，避免过长
            const desc = context.parentElement.content_desc;
            return desc.length > 20 ? desc.substring(0, 20) + '...' : desc;
          }
          // 4. 使用父元素的文本
          if (context.parentElement?.text) return context.parentElement.text;
          // 5. 使用 context 中增强后的文本（来自子元素或兄弟元素）
          if (context.elementText) return context.elementText;
          // 6. 使用子元素文本列表的第一个
          if (context.childrenTexts && context.childrenTexts.length > 0) {
            return context.childrenTexts[0];
          }
          // 7. 使用 resource_id（去掉包名前缀）
          if (uiElement.resource_id) {
            const parts = uiElement.resource_id.split('/');
            return parts[parts.length - 1] || uiElement.resource_id;
          }
          // 8. 最后回退到元素ID
          return uiElement.id;
        })();
