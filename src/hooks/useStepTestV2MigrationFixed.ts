...
export function useStepTestV2Migration() {
  ...
  const [executionMode, setExecutionMode] =
    useState<TestExecutionMode>("execute-step");
  const [testResults, setTestResults] = useState<
    Record<string, SingleStepTestResult>
  >({});
   ...
   * 🔧 内部执行逻辑，使用V2引擎
   */
  const executeStepInternal = useCallback(
    async (
      ...
      try {
        ...
        console.log(
          `✅ V1→V2迁移成功: ${
            v1CompatResult.success ? "执行成功" : "执行失败"
          }`
        );
  ...
  const executeStepWithMode = useCallback(
    async (
      step: SmartScriptStep,
      deviceId: string,
      mode?: TestExecutionMode
  ...
  const executeUnifiedStep = useCallback(
    async (
      step: SmartScriptStep,
      deviceId: string,
      mode?: TestExecutionMode
  ...
  const executeStrategyTest = useCallback(
    async (
      step: SmartScriptStep,
      deviceId: string
    ): Promise<SingleStepTestResult> => {
  ...
  // 📊 状态管理方法
  const getStepTestResult = useCallback(
    (stepId: string) => {
  ...
  const isStepTesting = useCallback(
    (stepId: string) => {
