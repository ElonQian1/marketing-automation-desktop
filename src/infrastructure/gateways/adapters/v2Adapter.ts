 ...
 * 将统一请求转换为V2协议
 */
export function convertToV2Request(request: V2ExecutionRequest): RunStepRequestV2 {
  const { deviceId, mode, actionParams, selectorId, bounds, verify, retry } = request;
  ...
  switch (actionParams.type) {
    ...
    case 'longPress':
      stepPayload = {
        ...
        offset: actionParams.params.offsetX || actionParams.params.offsetY ? {
    ...
    case 'swipe':
      stepPayload = {
        ...
        start: actionParams.params.startFrom === 'element' ? 'center' : 
               actionParams.params.startFrom === 'screenCenter' ? 'center' : 'custom',
    ...
    default:
      console.warn(`[V2Adapter] Unknown action type: ${(actionParams as StepActionParams).type}`);
      stepPayload = {
        ...baseStep,
        action: 'tap', // 默认降级
      };
