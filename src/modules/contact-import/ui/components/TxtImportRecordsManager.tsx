        ...
        <div style={{ fontSize: 12, lineHeight: 1.6 }}>
          ...
          <ul style={{ paddingLeft: 18, marginBottom: 12 }}>
            {selectedRecords.slice(0, 5).map(record => (
              <li key={record.id}>{record.file_name} ({record.imported_numbers} 个号码)</li>
            ))}
            {selectedRecords.length > 5 && <li>... 另外 {selectedRecords.length - 5} 个记录</li>}
          </ul>
        ...
        <Alert
          ...
          description={
            <div style={{ fontSize: '12px' }}>
              <p style={{ marginBottom: 4 }}>• <strong>直接删除</strong>：仅移除导入记录，保留号码当前状态</p>
              <p style={{ marginBottom: 0 }}>• <strong>归档删除</strong>：删除记录同时将相关号码重置为"未导入"状态，释放批次占用</p>
            </div>
          }
        />
