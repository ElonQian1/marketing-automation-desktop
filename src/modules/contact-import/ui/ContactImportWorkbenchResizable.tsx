...
export const ContactImportWorkbenchResizable: React.FC = () => {
  ...
  const [sessionsModal, setSessionsModal] = useState<{ open: boolean; deviceId?: string; status?: string }>({ open: false });
  ...
  const conflictPeersByDevice = useMemo(() => {
    const result: Record<string, { peerId: string; start: number; end: number; }[]> = {};
    for (const c of rangeConflicts) {
      if (!result[c.deviceA]) result[c.deviceA] = [];
      if (!result[c.deviceB]) result[c.deviceB] = [];
      result[c.deviceA].push({ peerId: c.deviceB, start: c.rangeB.start, end: c.rangeB.end });
      result[c.deviceB].push({ peerId: c.deviceA, start: c.rangeA.start, end: c.rangeA.end });
    }
    return result;
  }, [rangeConflicts]);
  ...
  const panelContents = [
    {
      ...
      content: (
        <DeviceAssignmentPanel
          ...
          onGenerateVcfForDevice={() => {/* TODO */}}
          onImportToDevice={() => {/* TODO */}}
          onOpenSessions={(params) => setSessionsModal({ open: true, ...params })}
          onGenerateBatches={() => {/* TODO */}}
          onTopLevelImportHint={() => message.info('请在下方设备卡片中执行具体操作')}
