...
import { ContactImporterUseCase, ContactImporterEventListener } from '../application/usecases/ContactImporterUseCase';
...
export function useContactImport(options: UseContactImportOptions = {}): UseContactImportReturn {
  ...
  // Refs
  const importerRef = useRef<ContactImporterUseCase | null>(null);
  ...
  const importContacts = useCallback(
    async (fileContent: string, targetDevices: Device[]): Promise<ImportResult> => {
      try {
        ...
        const importer = new ContactImporterUseCase({
          parser,
