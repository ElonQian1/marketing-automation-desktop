...
export function calculateTrend<T extends { created_at: Date }>(
  ...
  const currentPeriodItems = items.filter(
    (item) => item.created_at >= currentPeriodStart && item.created_at <= now
  );

  const previousPeriodItems = items.filter(
    (item) =>
      item.created_at >= previousPeriodStart &&
      item.created_at < currentPeriodStart
  );
