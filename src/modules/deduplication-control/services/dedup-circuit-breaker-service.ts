  ...
  static async getCircuitBreakerStatus(
    ...
    try {
      ...
      return {
        ...
        lastFailureTime: status.last_failure_time ? new Date(status.last_failure_time) : undefined,
  ...
  private async updateDedupCircuitBreakerState(
    ...
    switch (currentStatus.state) {
      ...
      case DedupCircuitBreakerState.HALF_OPEN:
        ...
        // 或者检查是否应该重新打开
        else if (this.decisionEngine.shouldOpenCircuit(
          stats.failureCount,
          stats.failureRate,
          stats.totalOperations
        )) {
    ...
    if (newState !== currentStatus.state) {
      ...
      currentStatus.nextCheckTime = this.decisionEngine.calculateNextCheckTime(newState);
    ...
    switch (status.state) {
      case DedupCircuitBreakerState.OPEN:
        ...
        if (status.nextCheckTime) {
          recommendations.push(`预计 ${status.nextCheckTime.toLocaleTimeString()} 后进入半开状态`);
        }
        break;
