...
export const SmokeTestCompletePage: React.FC = () => {
  ...
  const [elementContext, setElementContext] =
    useState<ElementSelectionContext | null>(null);
  ...
  const handleElementSelection = useCallback(async () => {
    ...
    logTestResult(
      "element-selection",
      `✅ 元素选择完成：${mockContext.elementText}`,
      "success"
    );
   ...
   * 阶段2：创建步骤卡片（默认值优先）
   */
  const handleStepCreation = useCallback(
    async (context: ElementSelectionContext) => {
   ...
   * 阶段3：智能分析（后台运行）
   */
  const handleIntelligentAnalysis = useCallback(
    async (context: ElementSelectionContext, stepId: string) => {
      ...
      <Card className="mb-6" title="🧪 智能分析工作流冒烟测试" bordered={false}>
        <Space direction="vertical" style={{ width: "100%" }}>
          ...
          <Paragraph>
            ...
            再到后台智能分析完成后的策略升级。重点验证
            <Text strong>默认值优先</Text>的设计理念。
          </Paragraph>
        </Space>
      </Card>
        ...
        <Steps
          ...
          items={[
            ...
            },
            {
              ...
              icon:
                testPhase === "step-creation" ? <LoadingOutlined /> : undefined,
            },
            {
              title: "智能分析",
            ...
            },
            {
              ...
              description: "验证完整工作流",
              icon:
                testPhase === "completed" ? <CheckCircleOutlined /> : undefined,
            },
          ]}
        />
      </Card>
