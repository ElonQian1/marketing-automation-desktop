 ...
 * 创建模拟步骤卡片数据
 */
const createMockStepCard = (state: IntelligentStepCard['analysisState']): IntelligentStepCard => {
  const baseCard: IntelligentStepCard = {
    ...
    strategyMode: state === 'analysis_completed' ? 'intelligent' : 'static_user',
    ...
    activeStrategy: {
      ...
      description: state === 'analysis_completed' ? '基于元素自身属性定位' : '使用完整的XPath路径（兜底策略）',
      variant: state === 'analysis_completed' ? 'self_anchor' : 'index_fallback',
      enabled: true,
...
export const UnifiedStepCardDemo: React.FC = () => {
   ...
   * 创建不同状态的演示卡片
   */
  const createDemoCard = useCallback((state: IntelligentStepCard['analysisState']) => {
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <Card className="mb-6" title="🔧 统一步骤卡片组件演示" bordered={false}>
        <Space direction="vertical" style={{ width: '100%' }}>
          ...
          <Paragraph>
            ...
            <Text code>pending_analysis</Text>、
            <Text code>analyzing</Text>、
            <Text code>analysis_completed</Text>、
            <Text code>analysis_failed</Text>、
            <Text code>analysis_stale</Text>
          </Paragraph>
        </Space>
      </Card>
      ...
      <Card className="mb-6" title="控制面板" bordered={false}>
        <Space wrap>
          ...
          <Button
            danger
            onClick={() => createDemoCard('analysis_failed')}
          >
            创建分析失败卡片
          </Button>
          ...
          <Col xs={24} md={12}>
            <Card size="small" title="分析状态" type="inner">
              <ul className="text-sm">
                <li><Text code>analyzing</Text> - 蓝色状态条，显示进度</li>
                <li><Text code>analysis_completed</Text> - 绿色/橙色，显示升级选项</li>
                <li><Text code>analysis_failed</Text> - 红色状态条，提供重试</li>
                <li><Text code>analysis_stale</Text> - 灰色/黄色，提示过期</li>
              </ul>
            </Card>
          </Col>
          <Col xs={24} md={12}>
            <Card size="small" title="策略模式" type="inner">
              <ul className="text-sm">
                <li><Text code>intelligent</Text> - 🧠 智能匹配（组合）</li>
                <li><Text code>smart_variant</Text> - ⚡ 智能-单步固定</li>
                <li><Text code>static_user</Text> - 🔧 用户自建静态</li>
              </ul>
            </Card>
          </Col>
        </Row>
            ...
            {demoCards.map((card, index) => (
              <UnifiedStepCard
                ...
                onSwitchStrategy={(strategyKey) => {
                  const strategy = card.smartCandidates.find(c => c.key === strategyKey);
                  if (strategy) {
                    setDemoCards(prev => prev.map(c =>
                      c.stepId === card.stepId ? { ...c, activeStrategy: strategy } : c
                    ));
                ...
                onCancelAnalysis={() => {
                  setDemoCards(prev => prev.map(c =>
                    c.stepId === card.stepId
                      ? { ...c, analysisState: 'idle' as const, analysisProgress: 0 }
                      : c
                  ));
                  message.info('已取消分析');
                }}
