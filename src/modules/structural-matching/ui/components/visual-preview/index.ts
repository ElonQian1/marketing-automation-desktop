// src/modules/structural-matching/ui/components/visual-preview/index.ts
// module: structural-matching | layer: ui | role: visual-preview-exports
// summary: 可视化预览组件导出文件

export { FloatingVisualOverlay } from './floating-visual-overlay-adapter';
export type { FloatingVisualOverlayProps } from './floating-visual-overlay-adapter';

export { useTreeVisualCoordination } from './use-tree-visual-coordination';
export type { UseTreeVisualCoordinationProps } from './use-tree-visual-coordination';