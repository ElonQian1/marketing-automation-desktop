...
// summary: 从步骤卡片数据加载XML和截图的Hook (支持硬编码开发模式)
...
export function useStepCardData(stepCardData?: StepCardData) {
  ...
  const parseElementTreeData = useCallback(
    async (
      ...
      console.log("🚨 [useStepCardData] 准备执行边界修正...", {
      ...
      console.log("🚨 [useStepCardData] 边界修正结果:", correctionResult);
