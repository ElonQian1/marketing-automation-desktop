        ...
        <div className={`${styles.toolbar}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              title="收藏/取消收藏当前搜索"
              onClick={() => {
                const ok = toggleFavoriteSearch(filter);
                setFavSearch(getFavoriteSearches());
              }}
            >
            ...
            <button className={styles.btn} onClick={() => setExpandAll(true)}>
              展开全部
            </button>
            <button
              className={styles.btn}
            ...
            <button className={styles.btn} onClick={locateFirstMatch}>
              定位匹配
            </button>
            <button className={styles.btn} onClick={() => goToMatch(-1)}>
              上一个
            </button>
            <button className={styles.btn} onClick={() => goToMatch(1)}>
              下一个
            ...
            <MatchCountSummary
              ...
              index={matchIndex}
              autoSelectOnParse={autoSelectOnParse}
              onToggleAutoSelect={setAutoSelectOnParse}
            />
          </div>
        </div>
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          ...
          <label className="text-xs text-neutral-500 flex items-center gap-1">
            <input
              ...
              onChange={(e) =>
                setSearchOptions((s) => ({ ...s, useRegex: e.target.checked }))
              }
            />{" "}
            使用正则
          </label>
          ...
          <select
            ...
            value={expandDepth}
            onChange={(e) => {
              setExpandDepth(parseInt(e.target.value, 10) || 0);
              setExpandAll(false);
            }}
            style={{ width: 96 }}
          >
          ...
          <div className="relative">
            ...
            <datalist id="grid-xpath-history">
              {favXPath.map((s, i) => (
                <option key={`favx-${i}`} value={s} />
              ))}
              {xpathHistory
                .filter((s) => !favXPath.includes(s))
                .map((s, i) => (
                  <option key={`xh-${i}`} value={s} />
                ))}
            </datalist>
          </div>
          <button
            className={styles.btn}
            title="收藏/取消收藏当前 XPath"
            onClick={() => {
              const ok = toggleFavoriteXPath(xPathInput);
              setFavXPath(getFavoriteXPaths());
            }}
          >
            {favXPath.includes(xPathInput.trim()) ? "★" : "☆"}
          </button>
          <button
            className={styles.btn}
            title="清空 XPath 历史"
          ...
          <button className={styles.btn} onClick={locateXPath}>
            定位 XPath
          </button>
          <span className="mx-2 h-4 w-px bg-neutral-200 dark:bg-neutral-800" />
          <button
            className={styles.btn}
            onClick={() => {
              const t = findNearestClickableAncestor(selected);
              if (t) setSelected(t);
            }}
          >
            选中可点击父级
              ...
              {root ? (
                <TreeRow
                  ...
                  hasActiveFilter={
                    ...
                    Boolean(
                      advFilter.enabled &&
                        (advFilter.resourceId ||
                          advFilter.text ||
                          advFilter.className ||
                          advFilter.packageName ||
                          advFilter.clickable !== null ||
                          advFilter.nodeEnabled !== null)
                    )
                  }
            ...
            {selected && (
              <div className="p-2 border-t border-[var(--g-border)] flex items-center justify-end">
                ...
                <ScreenPreviewSetElementButton
                  node={selected}
                  onApply={onApplyCriteria as any}
                />
              </div>
            )}
          </div>
        ...
        <div className="space-y-4">
          ...
          <NodeDetailPanel
            ...
            onApplyToStepComplete={(complete) => {
              console.log(
                "🎯 [GridElementView] onApplyToStepComplete 被调用，complete:",
                complete
              );
