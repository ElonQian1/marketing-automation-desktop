 ...
 * 专门处理可视化视图中的元素交互逻辑
 */
export const useVisualElementInteractionManager = (
  elements: UIElement[],
  ...
  // 当前选中但未确认的元素
  const [pendingSelection, setPendingSelection] = useState<SelectedElementState | null>(null);
  ...
  const handleElementClick = useCallback((element: UIElement, clickPosition: { x: number; y: number }) => {
    ...
    console.log('🎯 元素点击:', element.id, element.text);
  ...
  }, [enableHover, hoverDelay, isElementHidden]);

  // 确认选择元素
  const confirmSelection = useCallback(() => {
    if (!pendingSelection) return;

    console.log('✅ 确认选择元素:', pendingSelection.element.id);
  ...
  const hideElement = useCallback(() => {
    ...
    const restoreTimeout = setTimeout(() => {
      console.log('🔄 自动恢复隐藏元素:', elementId);
  ...
  const restoreElement = useCallback((elementId: string) => {
    console.log('🔄 手动恢复隐藏元素:', elementId);
