# Universal Mobile App Automation Platform

一个使用 Tauri + React + Rust 构建的通用移动应用自动化平台，专为营销自动化和企业级移动应用管理而设计。

## 🎯 项目概述

本项目是一个**通用移动应用自动化平台**，支持：

### 🚀 核心功能
- **跨应用自动化** - 支持任何Android应用的UI自动化
- **智能元素匹配** - 基于AI的元素识别和匹配策略
- **可视化脚本构建** - 拖拽式自动化流程设计
- **多设备管理** - 同时管理和操作多台设备
- **企业级联系人系统** - 批量联系人导入和管理

### 📱 应用场景
- **社交媒体营销自动化** - 自动关注、点赞、评论等
- **企业客户关系管理** - 批量联系人导入和分发
- **移动应用测试** - UI自动化测试和回归测试
- **数据采集和监控** - 移动端数据抓取和监控
...
### 🏗️ 技术特色
- **DDD架构设计** - 领域驱动的模块化架构
- **统一ADB接口** - 通过 `useAdb()` 管理所有设备操作
- **跨设备兼容** - 支持不同品牌、分辨率的Android设备
- **实时设备监控** - 自动检测设备连接状态变化
