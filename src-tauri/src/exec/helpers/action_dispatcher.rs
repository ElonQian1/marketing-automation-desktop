// src-tauri/src/exec/helpers/action_dispatcher.rs
// module: exec | layer: helpers | role: 动作分发器
// summary: 处理无需元素匹配的直接动作（wait, keyevent, back, coordinate-swipe）

use serde_json::Value;
use crate::exec::helpers::step_executor::{execute_keyevent_action, execute_swipe_action};

/// 尝试执行非元素交互动作
/// 
/// 如果动作类型是 wait, keyevent, back 或 纯坐标swipe，则直接执行并返回结果。
/// 返回：
/// - Ok(Some(result)): 动作已执行，返回结果
...
pub async fn try_dispatch_direct_action(
    ...
    if action_type == "keyevent" {
        ...
        let result = execute_keyevent_action(device_id, keycode, step_id).await?;
        return Ok(Some(result));
    }

    if action_type == "back" {
        let result = execute_keyevent_action(device_id, 4, step_id).await?;
        return Ok(Some(result));
    }
    ...
    if action_type == "swipe" {
        ...
        if let (Some(sx), Some(sy), Some(ex), Some(ey)) = (
            ...
            params.get("end_y").and_then(|v| v.as_i64())
             ...
             let duration = params.get("duration").and_then(|v| v.as_i64()).unwrap_or(1000) as i32;
             let result = execute_swipe_action(device_id, sx as i32, sy as i32, ex as i32, ey as i32, duration as i32, step_id).await?;
             return Ok(Some(result));
        }
