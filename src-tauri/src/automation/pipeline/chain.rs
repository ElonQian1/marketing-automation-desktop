...
use crate::automation::events::{emit_complete, emit_progress};
...
// use crate::exec::recovery_manager::{RecoveryContext, attempt_recovery};
...
use crate::exec::helpers::execution_tracker;

// 🆕 导入helpers模块中的设备管理功能（避免代码重复）
use crate::exec::helpers::device_manager;

// 🆕 导入helpers模块中的步骤执行功能（避免代码重复）
use crate::exec::helpers::batch_executor;

// 🆕 导入helpers模块中的智能分析辅助功能（避免代码重复）
use crate::exec::helpers::analysis_helpers;

// 🆕 导入helpers模块中的步骤评分功能（避免代码重复）
use crate::exec::helpers::step_scoring;

// 🆕 导入helpers模块中的Phase处理功能（避免代码重复）
use crate::exec::helpers::phase_handlers::{handle_intelligent_fallback, score_steps_by_mode};

// 🆕 导入helpers模块中的智能预处理功能（避免代码重复）
use crate::exec::helpers::intelligent_preprocessing::{
...
fn execute_chain_by_inline<'a>(
    ...
    Box::pin(async move {
        ...
        let screen_hash = crate::exec::helpers::device_manager::calculate_screen_hash(&ui_xml);
