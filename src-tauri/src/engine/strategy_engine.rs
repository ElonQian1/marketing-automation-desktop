...
// ✅ Step 1-10 智能策略分析核心引擎 - 统一评分逻辑，避免重复计算
//
// ┌─────────────────────────────────────────────────────────────────────────────┐
// │  🎯 V3架构 - 统一步骤序号体系（10步策略）                                      │
// │                                                                             │
// │  🏆 Tier 1: 批量结构匹配（瀑布流场景首选）                                     │
// │    Step 1 - card_subtree_scoring  卡片子树评分     (UnifiedMatchService)     │
// │    Step 2 - leaf_context_scoring  叶子上下文评分   (UnifiedMatchService)     │
// │                                                                             │
// │  🏆 Tier 2: 结构定位兜底                                                     │
// │    Step 3 - index_fallback        索引路径定位     (此文件)                  │
// │    Step 4 - region_scoped         区域约束策略     (此文件)                  │
// │                                                                             │
// │  🏆 Tier 3: 语义精确匹配（唯一元素场景）                                       │
// │    Step 5 - text_exact_scoring    文本唯一匹配     (UnifiedMatchService)     │
// │    Step 6 - heuristic_id_scoring  ID稳定性评分     (UnifiedMatchService)     │
// │    Step 7 - content_desc          描述文本定位     (此文件)                  │
// │                                                                             │
// │  🏆 Tier 4: 传统XPath兜底（最后保障）                                         │
// │    Step 8 - self_anchor           自锚定策略       (此文件)                  │
// │    Step 9 - heuristic_xpath_scoring XPath启发生成 (UnifiedMatchService)     │
// │    Step 10 - xpath_fallback       XPath兜底策略    (此文件)                  │
// │                                                                             │
// │  📍 同步文件：                                                               │
// │    - 前端: src/config/step-sequence.ts                                      │
