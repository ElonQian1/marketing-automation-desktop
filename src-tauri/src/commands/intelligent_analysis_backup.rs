...
use crate::debug::{start_execution_session};
...
/// 简化的跟踪记录函数
fn trace_execution_step(session_id: &str, phase: &str, message: &str, data: serde_json::Value) {
    tracing::info!(
        "🔍 [TRACE:{}] {} - {} | 数据: {}",
        session_id,
        phase,
...
impl IntelligentAnalysisService {
    ...
    pub async fn start_analysis(
        ...
        // 1. 启动执行链路跟踪会话
        let session_id = start_execution_session(
            &config.element_context.element_text.clone().unwrap_or_default(),
            &config.selection_mode,
            &config.strategy
        );
        
        // 2. 开始记录执行链路
        trace_execution_step(
            &session_id,
            "FrontendTriggered",
            "智能分析启动",
            json!({
                ...
                "element_path": config.element_context.element_path,
                "element_text": config.element_context.element_text
            })
        );
        ...
        // 5. 保存任务
        {
            ...
            trace_phase!(session_id.clone(), ExecutionPhase::JobInitialization, LogLevel::Info, 
                "任务初始化完成", json!({
                    "job_id": job_id,
                    "active_jobs_count": jobs.len()
                })
            );
        }
        ...
        trace_phase!(session_id.clone(), ExecutionPhase::BackgroundTaskSpawn, LogLevel::Info, 
            "后台任务启动", json!({
                "job_id": job_id_clone,
                "workflow": "execute_analysis_workflow"
            })
        );
...
async fn execute_analysis_workflow(
    ...
    trace_phase!(session_id.clone(), ExecutionPhase::DataProcessing, LogLevel::Info, 
        "工作流开始", json!({
            "job_id": job_id,
            "selection_hash": selection_hash,
            "workflow": "execute_analysis_workflow"
    ...
    // Step 1: 初始化分析环境
    trace_phase!(session_id.clone(), ExecutionPhase::UIAnalysis, LogLevel::Info, 
        "初始化分析环境", json!({
            "job_id": job_id,
            "progress": 5,
            "step": "环境初始化"
    ...
    emit_progress(&app_handle, &job_id, &session_id, 5, "初始化分析环境").await;
    tokio::time::sleep(tokio::time::Duration::from_millis(200)).await;
    
    // Step 2: XML解析与结构分析 (主要工作量)
    trace_phase!(ExecutionPhase::UIAnalysis, "XML解析与结构分析", 
        json!({
            "job_id": job_id,
            "progress": 25,
            "element_path": config.element_context.element_path,
            "element_type": config.element_context.element_type
        })
    );
    ...
    // Step 3: 智能策略生成 (核心算法)
    trace_phase!(ExecutionPhase::StrategyGeneration, "智能策略生成", 
        json!({
            "job_id": job_id,
            "progress": 65,
            "strategy": config.strategy,
    ...
    // Step 4: 策略评分与优选
    trace_phase!(ExecutionPhase::StrategyGeneration, "策略评分与优选", 
        json!({
            "job_id": job_id,
            "progress": 85,
            "step": "质量评估"
    ...
    // Step 5: 生成最终分析报告
    trace_phase!(ExecutionPhase::ResultGeneration, "生成最终分析报告", 
        json!({
            "job_id": job_id,
            "progress": 95,
            "step": "报告生成"
    ...
    // 🆕 使用共用引擎生成真实的分析结果
    trace_phase!(ExecutionPhase::StrategyGeneration, "策略引擎分析", 
        json!({
            "job_id": job_id,
    ...
    trace_phase!(ExecutionPhase::ResultGeneration, "结果转换", 
        json!({
            "job_id": job_id,
            "confidence": step_result.confidence,
            "evidence_count": step_result.evidence.len(),
            "recommended_action": step_result.recommended_action
        })
    );
    ...
    trace_phase!(ExecutionPhase::ResultGeneration, "分析完成", 
        json!({
            "job_id": job_id,
            "progress": 100,
            "recommended_key": result.recommended_key,
            "confidence_percent": step_result.confidence * 100.0
        })
    );
    ...
    // 🆕 发送增强的完成事件 (包含置信度和证据)
    trace_phase!(ExecutionPhase::EventEmission, "发送完成事件", 
        json!({
            "job_id": job_id,
            "event": "analysis:done",
            "selection_hash": selection_hash,
            "element_uid": config.element_context.element_path,
            "card_id": config.step_id
        })
    );
...
/// 发送进度事件
async fn emit_progress(app_handle: &AppHandle, job_id: &str, session_id: &str, progress: u8, step: &str) {
    trace_phase!(ExecutionPhase::EventEmission, "进度事件发送", 
        json!({
            "job_id": job_id,
            "event": "analysis:progress",
            "progress": progress,
            "current_step": step,
            "estimated_time_left": ((100 - progress) as u64) * 50
        })
    );
