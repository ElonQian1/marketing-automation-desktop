...
impl SmartSelectionEngine {
    ...
    fn execute_batch_strategy(
        ...
        // 🎯 智能过滤：排除"已关注"状态的按钮（如果用户选择的是"关注"）
        let filtered_candidates: Vec<CandidateElement> = candidates.iter()
            .filter(|candidate| {
                // 如果是文本为"已关注"的按钮，在批量关注时应该跳过
                if let Some(text) = &candidate.element.text {
                    let is_followed = text.contains("已关注") || text.contains("已关注") || text.contains("Followed");
                    if is_followed {
                        debug_logs.push(format!("  跳过已关注按钮: \"{}\"", text));
                        return false;
                    }
                }
                
                // 如果是content-desc为"已关注"的按钮，也跳过
                if let Some(desc) = &candidate.element.content_desc {
                    let is_followed = desc.contains("已关注") || desc.contains("Followed");
                    if is_followed {
                        debug_logs.push(format!("  跳过已关注按钮(desc): \"{}\"", desc));
                        return false;
                    }
                }
    ...
    fn matches_text_criteria(element: &UIElement, protocol: &SmartSelectionProtocol) -> bool {
        // 🎯 首先检查目标文本的精确匹配
        if let Some(target_text) = &protocol.anchor.fingerprint.text_content {
        ...
        if let Some(context) = &protocol.matching_context {
            if let Some(aliases) = &context.i18n_aliases {
                ...
                if let Some(content_desc) = &element.content_desc {
                    return aliases.iter().any(|alias| content_desc.contains(alias));
                }
            }
        }
        
        // 🚨 如果没有配置目标文本和多语言别名，则拒绝匹配
        // 这防止了无目标的广泛搜索导致的错误匹配
        false
    }
    ...
    fn should_exclude(element: &UIElement, protocol: &SmartSelectionProtocol) -> bool {
        ...
        // � 智能互斥排除：根据目标文本自动排除对立按钮
        if auto_exclude_enabled {
            if let Some(target_text) = &protocol.anchor.fingerprint.text_content {
                // 当寻找"关注"时，排除"已关注"类型
                if target_text.contains("关注") && !target_text.contains("已") {
                    let exclude_patterns = &["已关注", "Following", "Followed", "互相关注", "已互关"];
                    if let Some(element_text) = &element.text {
                        for pattern in exclude_patterns {
                            if element_text.contains(pattern) {
                                debug!("🎯 智能排除：寻找'{}' -> 排除'{}'", target_text, element_text);
                                return true;
                            }
                        }
                    }
                    if let Some(content_desc) = &element.content_desc {
                        for pattern in exclude_patterns {
                            if content_desc.contains(pattern) {
                                debug!("🎯 智能排除：寻找'{}' -> 排除desc'{}'", target_text, content_desc);
                                return true;
                            }
                        }
                    }
                ...
                // 当寻找"已关注"时，排除"关注"类型
                if target_text.contains("已关注") {
                    let exclude_patterns = &["关注", "+关注", "Follow", "添加关注"];
                    if let Some(element_text) = &element.text {
                        for pattern in exclude_patterns {
                            // 🔥 精确排除：避免"已关注"被排除
                            if element_text == pattern || (element_text.contains(pattern) && !element_text.contains("已")) {
                                debug!("🎯 智能排除：寻找'{}' -> 排除'{}'", target_text, element_text);
                                return true;
                            }
                        }
                    ...
                    if let Some(content_desc) = &element.content_desc {
                        for pattern in exclude_patterns {
                            if content_desc == pattern || (content_desc.contains(pattern) && !content_desc.contains("已")) {
                                debug!("🎯 智能排除：寻找'{}' -> 排除desc'{}'", target_text, content_desc);
                                return true;
                            }
                        }
